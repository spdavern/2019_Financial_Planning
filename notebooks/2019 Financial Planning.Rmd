...
t.size <- 9
plot1 <- ggplot(df3, aes(x = week.ending, y = total, color=month )) +
  geom_point() + geom_line(color='blue', data = df3, 
...
plot4 <- ggplot(df3, aes(x = as.numeric(row.names(df3)), y = residual.values, 
  ...
  labs(x = "Row Number", y = "Residuals",
                  title = "Resid. by Row Plot") + theme(text = element_text(size=t.size)) +
  theme(legend.position="none")
plot1
