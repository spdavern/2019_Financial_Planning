...
```{r, include=FALSE}
# Needed this code to get more detail on why knitting pdf was failing.
# Source: https://yihui.name/tinytex/r/#debugging
options(tinytex.verbose = TRUE)
# Solution came from: https://stackoverflow.com/questions/26049762/erroneous-nesting-of-equation-structures-in-using-beginalign-in-a-multi-l
```
...
&= a+(\overline{b_{year}}+b_{year}-\overline{b_{year}})+
(\overline{c_{month}}+c_{month}-\overline{c_{month}})
\\
...
\end{aligned}
$$
where $\overline{b_{year}}$ is the mean of $b_{year}$ coefficients, including $b_{2010}=0$, and $\overline{c_{month}}$ is the mean of the $c_{year}$ coefficients, including $c_{January}=0$. The term $\big[a+\overline{b_{year}}+\overline{c_{month}}\big]$ is then the regressed $\textit{Monthly Giving}$ for an average month in an average year over the period covered by the data.  Of course no actual month or year is the average, and to reproduce a given month and year prediction the shifted coefficients $(b_{year}-\overline{b_{year}}) \text{ and } (c_{month}-\overline{c_{month}})$ must be employed.  However, this facilitates using the model for monthly variance predictions in the upcoming year since:
$$
\begin{aligned}
\begin{split}
\textit{Average Monthly Giving}_\textit{next year}
&=\big[a+\overline{b_{year}}+\overline{c_{month}}\big]+\big(b_\textit{next year}-\overline{b_{year}}\big)
\\
&=\frac{\textit{Projected Income}_\textit{next year}}{12}
\end{split}
\end{aligned}
$$
So even though we don't know $b_\textit{next year}$ we can make an estimation for next year's total $\textit{Projected Income}$ and get monthly estimates based on the annual estimate:

$$
\textit{Monthly Giving}_\textit{next year}
=\frac{\textit{Projected Income}_\textit{next year}}{12} + 
\big(c_{month}-\overline{c_{month}}\big)
$$
...
```{r echo=FALSE}
mon.coef <- c(0, mod$coefficients[5:15])
